...
from app.graph.nodes.specialists import water_analysis_node, knowledge_node, vision_analysis_node
...
# Steps flow representing planning -> analysis workers -> validation -> reporting
...
workflow.add_edge("vision_analysis", "knowledge")
...
def route_reflection(state: AgentState) -> str:
    ...
    if is_valid or iterations >= 3:
        ...
        logger.warning(f"Reflection loop failed. Iteration count={iterations}. Routing back to Water Analysis.")
...
logger.info("LangGraph workflow compiled successfully for Milestone 4.")
