...
const Login = ({ onLoginSuccess, switchToRegister }) => {
  ...
  React.useEffect(() => {
    // Check if the URL has an error hash (e.g. from an expired email link)
    if (window.location.hash) {
      ...
      if (errorDescription) {
        ...
        if (errorCode === 'otp_expired') {
          setError(`${formattedError}. Please enter your email below and click "Resend Confirmation Email".`);
  ...
  const handleResend = async () => {
    if (!email) {
      setError('Please enter your email below to resend the link.');
      return;
    }
    setError('');
    setSuccessMessage('');
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-6 select-none">
      <div className="w-full max-w-md bg-slate-900/60 rounded-3xl border border-slate-800 p-8 space-y-6 backdrop-blur-md shadow-2xl">
        ...
        {error && (
          <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs flex flex-col gap-2">
            ...
            {needsConfirmation && (
              <button 
                ...
                className="self-start text-xs font-bold text-aqua-400 hover:underline"
              >
